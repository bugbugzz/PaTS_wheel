% function [sigma_total, sigma_axial, sigma_bending, SF] = calculateLinkStresses(Qc, b, h, yield_strength, L_vals)
%     [~, num_steps] = size(Qc);
%     sigma_axial = zeros(5, num_steps);
%     sigma_bending = zeros(5, num_steps);
%     sigma_total = zeros(5, num_steps);
% 
%     % Section Properties for the TPU Flexure Notch
%     A = b * h;               % Area [m^2]
...
% 
%         % 2. REAL Bending Stress: Force * Lever Arm to the hinge
...
function [sigma_total, sigma_axial, sigma_bending, SF] = calculateLinkStresses(Qc, b, h_array, yield_strength, L_vals)
    ...
    for i = 1:5
        % --- NEW: Calculate properties specific to THIS hinge's thickness ---
        h_current = h_array(i);
